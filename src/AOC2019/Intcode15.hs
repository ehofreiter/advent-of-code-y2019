...
liftInterrupt
  :: (Monad m, MonadError Interrupt m)
...
pmHalt = ProgramT $ throwError InterruptHalt
...
pmAdvance offset = ProgramT $ modify' $ overInstrPtr (+ A offset)

pmJump :: Monad m => Address -> ProgramT m ()
pmJump addr = ProgramT $ modify' $ overInstrPtr (const addr)

pmOffsetRelBase :: Monad m => Integer -> ProgramT m ()
pmOffsetRelBase offset = ProgramT $ modify' $ overRelBase (+ A offset)

pmReadInput :: Monad m => ProgramT m Integer
pmReadInput = do
  readInput <- ProgramT ask
  lift readInput

pmWriteOutput :: Monad m => Integer -> ProgramT m ()
pmWriteOutput i = ProgramT $ tell [i]
